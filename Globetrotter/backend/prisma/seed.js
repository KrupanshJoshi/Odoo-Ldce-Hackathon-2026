...
async function main() {
  ...
  const cities = [
    ...
    { name: 'Ahmedabad', country: 'India', region: 'Asia', costIndex: 20, popularity: 70, imageUrl: 'https://images.unsplash.com/photo-1621765527740-8c8c4e33cdb3' },
  ...
  const activityTemplates = (cityName) => [
    { name: `${cityName} Walking Tour`, category: 'SIGHTSEEING', cost: 15, durationMin: 120, description: `Guided walking tour through ${cityName}'s highlights.` },
    { name: `${cityName} Food Crawl`, category: 'FOOD', cost: 35, durationMin: 150, description: `Taste local specialties around ${cityName}.` },
    { name: `${cityName} Museum Pass`, category: 'CULTURE', cost: 20, durationMin: 180, description: `Entry to top museums in ${cityName}.` },
    { name: `${cityName} Adventure Day`, category: 'ADVENTURE', cost: 60, durationMin: 300, description: `Full-day adventure activity near ${cityName}.` },
    { name: `${cityName} Nightlife Experience`, category: 'NIGHTLIFE', cost: 40, durationMin: 180, description: `Evening out at ${cityName}'s best spots.` },
  ];

  for (const [name, city] of Object.entries(cityRecords)) {
    for (const act of activityTemplates(name)) {
